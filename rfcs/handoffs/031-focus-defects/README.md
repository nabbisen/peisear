...
| A11Y-004 | [A11Y-004](./A11Y-004-the-toast-beside-the-control-and-a-restore.md) | `A11Y-003`'s ruling: the toast moves beside the control in DOM order, and focus returns to that control when the toast ends **if focus is inside it**. **Shape 1 — focusing the toast — is rejected**: it fixed every measurement and made a second Enter on a status button silently undo the change. | 0.41.0 |
