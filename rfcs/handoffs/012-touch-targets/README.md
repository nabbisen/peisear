...
| TT-005 | [TT-005](./TT-005-adjacency-measured.md) | ✅ Done. `§10.19` — the two `<summary>` overlaps **do not exist**, and not on the 2026-09-06 tree they were measured from either. Withdrawn like `§10.21`; the `join` seam is real and the requirement now carves it out. No source change. | 0.34.0 |
| TT-006 | [TT-006](./TT-006-the-guard-doc-denies-its-own-exception-list.md) | ✅ Done. `§10.28` — the guard's doc denied the exception list the module has carried since `TT-004`. Corrected; doc only, count unmoved. | 0.34.0 |
