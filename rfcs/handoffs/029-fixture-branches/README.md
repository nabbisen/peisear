...
| ~~GATE-004~~ | [GATE-004](./GATE-004-the-inbox-sweeps-its-empty-state.md) | **WITHDRAWN.** `/inbox` is swept with **no notifications**, so it renders its empty state at all five widths while notification rows carry issue and project text. Not doable: no route emits a notification (the snapshot job's detectors need days, not a second actor), **and the rows carry no user text** — fixed copy with a count. `§10.32`'s stopping rule applied on its first test. | — |
