...
| **0.40.0 (this baseline)** | **No screen, route, state or copy changed.** Three surfaces stop overflowing on user text: the sprint plan's backlog filter, whose project and assignee `<select>`s took their longest option's width (729 px and 626 px inside a 320 px viewport), and the assignee name wherever it appears — the board card, the workload chip and issue detail — which **overflowed a 1280 px desktop**, the second desktop overflow after `§10.27`'s. Both are `§10.25`'s classes on elements rather than text nodes: a `<select>` yields to `min-w-0`; a `badge` needs `wrap-anywhere` **and** `h-auto`, because a fixed-height pill spills a wrapped name over its neighbour — a defect no overflow measurement sees, found by looking at the rendering. **What changed for a reader of this document is the coverage behind it**: `BROWSER-001` went 95 → 120 cells, and `§17.8`'s caveat is now the register's `§10.32` — a page can only overflow on content it renders, and this fixture three times did not render the branch a defect sat behind |
| 0.39.0 | **`SCR-20` gains a third lifecycle control and two headings.** A completed sprint's page offers **Reopen sprint** beside Delete — a plain form, no interstitial, because it is reversible — and labels its two halves: *Summary at completion* over figures that no longer move, and *Issues in this sprint now* over a list that still can. **The pair exists because the two disagree by design**: an issue carried out of a completed sprint leaves the list while the captured figures still count it, and a page showing both without saying which is which would read as a contradiction. **One surface shows less**: the velocity median on `SCR-19` is suppressed unless a single sprint in the window had two contributors, where the union of contributors used to decide it. **No route is removed**; `POST …/sprints/{id}/reopen` is added. The overflow gate reaches **nineteen** pages, a completed sprint's detail page having been added rather than swapped for the planned one, whose plan page carries the backlog and its drag controls |
