...
**Covers release**: `0.39.0` (**`FR-SPR-004`'s mechanism replaced** — a
completed sprint's record is captured at completion rather than its membership
frozen, RFC 0013 / `DEC-054`; five check-then-write races closed including the
optimistic lock itself; `NFR-PRIV-007`'s floor narrowed; implementation
through `0.39.0`)
