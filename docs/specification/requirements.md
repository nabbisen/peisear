...
**Covers release**: `0.40.0` (**the record audited against the code** —
sixteen statuses corrected, a claimed gap that did not exist, and two entries
...
| **0.40.0 (this baseline)** | **The release where the record was checked against the code rather than trusted.** `REQ-001` audited every requirement whose status is a claim: **sixteen were stale**, all understating what ships — three `Specified` entries picked at random before the audit was even commissioned were all shipped. **One was in the dangerous direction**: `FR-HLT-006` recorded that no automated vocabulary guard exists, and one does. **Two entries contradicted each other** about whether the health score badge still exists. The audit's more valuable half was **six requirements whose text admits two readings** where the status silently took one — the shape that cost two handoffs at `FR-SPR-004` a release earlier, now an obligation beside `§10.17`. **Three layout defects**, all on user text in containers that could not shrink: the sprint plan's filter `<select>`s (729 px in a 320 px viewport, on a page the gate had swept green for eleven releases) and the assignee badge, which **overflowed a 1280 px desktop**. **`§10.32` opens**: the gate's fixture had three times not rendered the branch a defect sat behind, and the fixture grew 95 → 120 cells across four changes, each finding the next — with a stopping rule, tested and applied at `GATE-004`, which was withdrawn under it. **`FR-CAL-007`, a P0, reaches Met**: its behaviour always held while its mandated guard checked a different property. `PRAGMA optimize` runs on connection close — **not for the 7× it buys on one query, but because install shapes cannot be predicted**. **No migration. Test inventory 346 → 352** |
| 0.39.0 | **A completed sprint's figures stop moving, and the requirement that was supposed to stop them is replaced.** `FR-SPR-004` protected *membership*, which is **one of four inputs** to the figures it exists to protect — `summary` reads each member's current `status`, `burndown` its `status`, `effort` and `updated_at` — so a completed sprint drifted when a carried-over issue was finished a month later. Two handoffs written against the old wording would have blocked carry-over, the designed path for unfinished work; **the dev team stopped the second before writing code, having measured the flow it would break**, and half of the first was reverted. RFC 0013 (`DEC-054`) **captures the record at completion**; reopen (`DEC-053`) discards it so a sprint completed by mistake can be corrected. **Five check-then-write races close**, among them the last-admin guard — two administrators leaving at once left a team with **no administrator** — and **the optimistic lock itself**, which compared a value the handler had read earlier. `§10.31` records why the transactions they replaced were *failing* unrelated concurrent writes, which reframes their cost. **`NFR-PRIV-007` narrows twice**: a completed sprint's trajectory is gated on the contributors it had at completion, and an aggregate passes only if one sprint clears the floor alone. **Test inventory 297 → 346**; the overflow gate 90 → 95 cells |
| 0.38.0 | **`FR-DM-001` is amended from five direct-manipulation surfaces to four and reaches Met.** Issue-list reordering (D-5) is **retired by owner decision** rather than built — the product already answers *what is next* with priority bands, sprint membership and planned dates, and the sprint is the better answer; a manual order would have been a second answer with no name in the UI and no visible provenance, and RFC 0004's requirement 10 makes the affordance per-row buttons on a phone. **RFC 0004 closes at four of five.** `ORD-001` removes the `position` column it would have used (migration `0018`, and `Issue.position` leaves `peisear-core`'s public API). **Four ordering defects close as one shape** (`§10.29`): a TEXT column ordered alphabetically as though that meant severity or lifecycle, three times, and 22 timestamp orderings carrying no tiebreak, three of which resolved a user's current capacity and returned the **superseded** value. **One concurrency defect closes** (`§10.30`): the capacity overlap check and its write were not atomic, and the comment saying that could not happen was wrong about what WAL serializes. `FR-DM-001` also carries a **second correction** — its own per-surface breakdown, added so a stale reading could not recur, was itself stale for four releases, and the entry now states the obligation the first correction omitted. **The suite found none of the five defects**; 272 → 297 tests. `DEC-020` closed at 0.38.0 and this document moved into the repository |
